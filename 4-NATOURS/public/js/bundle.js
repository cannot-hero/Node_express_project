...
var hideAlert = function hideAlert() {
  var el = document.querySelector('.alert'); // 删除dom，需要移到父元素，然后删除子元素
...
var showAlert = function showAlert(type, msg) {
  // 首先隐藏已存在的警报
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); // 插入相邻的HTML afterbegin 表示在一开始插入  markup表示想要包含的HTML
        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', 'Login successfully!');
...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    ...
    scrollZoom: false // center: [-118.113, 34.111],
    // zoom: 10
...
//前端使用公钥
var stripe = Stripe('pk_test_51LdvapATwayfKpSDiXmeSlhJj4ekmwCxoNJn2Rq3qTeZz6TR0S6XOMzUyFomegHqUuNbzN1zxIylUNbU11suIwJr00TItTF562');
...
var bookBtn = document.getElementById('book-tour'); // 委派 delegation
...
if (logOutBtn) logOutBtn.addEventListener('click', _login.logout); // update user data
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              passwordConfirm = document.getElementById('password-confirm').value; // 要在更新密码之后清除页面中相应字段， await等待这个完成
